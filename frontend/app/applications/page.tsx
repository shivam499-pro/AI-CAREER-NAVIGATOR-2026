...
export default function ApplicationsPage() {
  ...
  if (loading) {
    return (
      ...
      <Loader2 className="w-12 h-12 animate-spin text-[#C2652A] mx-auto mb-4" />
  ...
  return (
    <div className="min-h-screen bg-[#1A1410] text-white selection:bg-[#C2652A]/30">
      <Navbar />
      <main className="max-w-6xl mx-auto px-4 py-12">
        <motion.div initial="hidden" animate="visible" variants={{
          ...
          }} className="bg-[#211A14] rounded-[2.5rem] p-10 mb-12 border border-white/5 relative overflow-hidden group shadow-2xl">
            <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-[#C2652A]/10 to-transparent pointer-events-none" />
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
              <div>
                <div className="flex items-center gap-3 mb-6">
                  <div className="p-3 bg-[#C2652A]/20 rounded-2xl border border-[#C2652A]/30 shadow-[0_0_20px_rgba(194,101,42,0.3)]">
                    <Briefcase className="w-8 h-8 text-[#C2652A]" />
                  </div>
                  <div>
                    <h1 className="text-4xl font-black tracking-tighter uppercase italic">Application <span className="text-[#E8A55C]">Tracker</span></h1>
            ...
            {Object.entries(STATUS_CONFIG).map(([status, config]) => {
              ...
              return (
                <motion.div
                  ...
                  className={`bg-[#211A14] rounded-3xl p-6 border cursor-pointer transition-all ${
                    filter === status 
                      ? 'border-[#C2652A]/50 shadow-[0_10px_30px_-10px_rgba(194,101,42,0.3)]' 
          ...
          <motion.section variants={{
            ...
            <div className="flex items-center justify-between mb-8 px-2">
              <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-[#E8A55C]" /> {filter ? `${STATUS_CONFIG[filter as keyof typeof STATUS_CONFIG].label} Applications` : 'All Applications'}
              </h3>
              {filter && (
                <Button
                  variant="outline"
                  ...
                  className="bg-[#211A14] border-white/10 hover:border-[#C2652A]/30 text-slate-400 hover:text-white rounded-xl py-3 px-4 font-black uppercase tracking-widest text-[10px]"
                >
                  Clear Filter
                </Button>
              )}
            ...
            {applicationsLoading ? (
              <div className="bg-[#211A14] rounded-[2rem] border border-white/5 p-20 text-center flex flex-col items-center shadow-xl">
                <div className="w-16 h-16 rounded-full border-4 border-[#C2652A]/20 border-t-[#C2652A] animate-spin mb-6" />
                  ...
                  {applications.map((app, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="bg-[#211A14] rounded-[2rem] border border-white/5 p-6 relative overflow-hidden group hover:border-[#C2652A]/30 transition-all shadow-xl"
                      >
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-xs font-black text-slate-400 uppercase tracking-tighter">
                              <div className="flex items-center gap-2">
                                <Building2 className="w-3 h-3 text-[#C2652A]" /> {app.company || 'Unknown Company'}
                              </div>
                              <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="relative group/menu">
                              <Button
                                ...
                                className="bg-[#1A1410] border-white/10 hover:border-[#C2652A]/30 text-slate-400 hover:text-white rounded-xl py-4 px-4 font-black uppercase tracking-widest text-[10px]"
                              >
                              ...
                              <div className="absolute right-0 top-full mt-2 w-48 bg-[#211A14] border border-white/10 rounded-xl shadow-xl opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all z-10">
                                {Object.entries(STATUS_CONFIG).map(([status, config]) => (
                                  <button
                                    key={status}
                                    ...
                                    className={`w-full text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-white/5 first:rounded-t-xl last:rounded-b-xl ${
                                      app.status === status ? 'text-[#E8A55C]' : 'text-slate-400'
                                    }`}
                                  >
                            ...
                            {app.apply_url && app.apply_url !== '#' && (
                              <Button
                                size="sm"
                                className="bg-[#C2652A] hover:bg-[#C2652A]/90 text-white font-black uppercase tracking-tighter py-4 px-4 rounded-xl text-[10px]"
              ...
              <div className="bg-[#211A14] rounded-[2rem] border border-white/5 border-dashed p-20 text-center flex flex-col items-center">
                ...
                <Button
                  className="mt-6 bg-[#C2652A] hover:bg-[#C2652A]/90 text-white font-black uppercase tracking-tighter py-4 px-8 rounded-xl text-xs"
