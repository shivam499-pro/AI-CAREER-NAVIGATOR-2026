  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [challengeRes, leaderboardRes] = await Promise.all([
        fetch(`${apiUrl}/api/v1/weekly-challenge/current`),
        fetch(`${apiUrl}/api/v1/weekly-challenge/leaderboard`)
      ])

      // ── Read challenge ONCE, save to variable ──────────────────
      let challengeData: WeeklyChallenge | null = null
      if (challengeRes.ok) {
        challengeData = await challengeRes.json()   // ← ONE read only
        setChallenge(challengeData)
      }

      // ── Read leaderboard ONCE, save to variable ────────────────
      let freshLeaderboard: LeaderboardEntry[] = []
      if (leaderboardRes.ok) {
        freshLeaderboard = await leaderboardRes.json()
      ...
      if (user && challengeData) {
        try {
          ...
          if (attemptRes.ok) {
            ...
            setAttemptStatus(
              attemptData.status === 'completed' ? 'completed' :
                attemptData.status === 'started' ? 'started' : 'none'
            )
          } else {
            // Fallback: check fresh leaderboard (not stale state)
                  ...
                  {error ? (
                    ...
                    leaderboard.map((entry) => (
                      <motion.div
                        key={entry.rank}
                        whileHover={{ x: 4 }}
                        className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${isCurrentUser(entry.user_email)
