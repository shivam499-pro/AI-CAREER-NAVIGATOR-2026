...
import { Loader2, Award, Zap, Lock, Sparkles } from 'lucide-react'
import Link from 'next/link'
import { Button } from '@/components/ui/button'
import { useBadges } from './hooks/useBadges'

// ─── Static animation variants (module scope — not recreated on every render) ──
...
function ErrorScreen({ message }: { message: string }) {
  return (
    <div className="min-h-screen bg-[#0F172A] flex items-center justify-center">
      <div className="text-center">
        <p className="text-red-400 font-bold text-lg mb-4">{message}</p>
        <Link href="/dashboard">
  ...
  return (
    ...
    >
      ...
      <p className="text-slate-500 font-bold max-w-sm mx-auto mb-10">
        Complete your first specialized interview session to authenticate your first milestone.
      </p>
      <Link href="/interview">
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        <motion.div initial="hidden" animate="visible" variants={containerVariants}>
          ...
          <div className="text-center mb-16 relative">
            <motion.div
              variants={itemVariants}
              className="inline-flex items-center gap-2 px-4 py-2 bg-purple-500/10 rounded-full border border-purple-500/20 text-[#6C3FC8] font-black uppercase tracking-widest text-[10px] mb-4"
            >
            ...
            <motion.h1
              variants={itemVariants}
              className="text-6xl font-black mb-6 tracking-tighter leading-tight"
            >
            ...
            <motion.div variants={itemVariants} className="max-w-2xl mx-auto">
              <div className="flex justify-between items-end mb-4 px-2">
                <span className="text-xs font-black uppercase tracking-widest text-[#6C3FC8]">
                  Completion Status
                </span>
              ...
              <p className="text-slate-400 mt-4 font-bold text-sm">
                Targeting {totalBadges} verified milestones across career intelligence.
              </p>
            </motion.div>
          </div>
          ...
          {earnedBadges.length > 0 && (
            <div className="mb-16">
              <div className="flex items-center gap-3 mb-8 px-2">
                ...
                <h2 className="text-sm font-black uppercase tracking-[0.3em] text-yellow-400">
                  Earned Milestones
                </h2>
                <div className="h-px bg-yellow-400/20 flex-1" />
              </div>
              ...
              >
                {lockedBadges.map((badge) => (
                  ...
                  >
                    ...
                    <h3 className="text-xl font-black text-slate-500 group-hover:text-slate-300 mb-2">
                      {badge.name}
                    </h3>
                    <p className="text-[10px] font-black uppercase tracking-widest text-[#EF4444] mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      Locked Achievement
                    </p>
                  </motion.div>
                ))}
          ...
          >
            <p className="text-slate-500 text-xs font-black uppercase tracking-widest mb-6">
              Want to unlock higher tiers?
            </p>
            <div className="flex justify-center gap-4">
              <Link href="/dashboard">
                <Button
                  variant="outline"
                  className="border-white/10 text-slate-400 hover:text-white font-black uppercase tracking-widest px-8 rounded-xl"
                >
                  Return Terminal
                </Button>
              </Link>
