...
export default function ProgressTracker() {
  ...
  const fetchProgress = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/api/v1/profile/progress`, {
        headers: {
    ...
    <div className="bg-[#1E293B] rounded-2xl p-8 border border-slate-800 shadow-[0_0_50px_rgba(108,63,200,0.1)] relative overflow-hidden group">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {(data.steps || []).map((step, i) => (
          <div
            key={i}
            className={`flex items-start gap-3 p-4 rounded-xl border transition-all duration-300 ${step.status === 'complete'
